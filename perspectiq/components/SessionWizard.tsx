...
const SessionWizard: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between mb-8 sticky top-0 bg-slate-50 dark:bg-black/95 backdrop-blur-sm py-4 z-30">
         <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Simulation</h1>
         </div>
         <div className="flex gap-3">
             {step > 1 && (
                 <button onClick={handleBack} className="pl-2 text-sm font-medium text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                     Back
                 </button>
             )}
             {step < 3 ? (
                 <button 
                    onClick={handleNext} 
                    disabled={!canProceed()}
                    className="px-4 py-2.5 bg-sky-500 hover:bg-sky-600 text-white rounded-full text-sm font-semibold shadow-lg shadow-sky-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2 cursor-pointer"
                 >
        ...
        <div className="relative flex justify-between">
           ...
           {steps.map((s) => (
               <div key={s.id} className="flex flex-col items-center bg-slate-50 dark:bg-black px-2">
                   ...
                   <span className={`mt-2 text-xs text-center font-semibold tracking-wide uppercase ${
